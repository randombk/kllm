/*
This file runs inference with the GPT-2 model.
...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) {
        ...
        printf("Example: %s \"Once upon a time\"\n", argv[0]);
        return 1;
    }
    ...
    if (!tokenizer.init_ok) {
        printf("Error: failed to initialize tokenizer\n");
        return 1;
    }
    ...
    memset(gen_tokens, 0, B * T * sizeof(int)); // Zero out the gen_tokens array
    ...
    tokenizer_encode(&tokenizer, argv[1], gen_tokens, &num_prompt_tokens);
    if (num_prompt_tokens == 0) {
        printf("Error: failed to encode prompt\n");
        return 1;
    }
    ...
    for (int t = num_prompt_tokens; t < max_tokens; t++) {
